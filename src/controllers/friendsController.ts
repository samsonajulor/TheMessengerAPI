...
export const getAllFriends = async (
  ...
  try {
    ...
    if (user.friends === [] || !user.friends)
      return res
        .status(400)
        .send("User does not have any friend. Add friends to your list");
    return res
      .status(200)
      .json({ status: "SUCCESS", friendList: user.friends });
  } catch (err: any) {
...
export const addFriend = async (
  ...
  const { email: friendEmail, _id} = req.body;
  const { error } = validateEmail(req.body);
  if (error) return res.status(400).send(error.details[0].message);
  try {
    const user: any = await User.findOne({ email: loginEmail }).exec(); //me
    let { firstName: first, lastName: last } = user;
    let userFullName = `${first} ${last}`;
    if (!user)
      return res
        .status(400)
        .json("User need to register to be able to add friend");
    const friendtoBe: any = await User.findOne({ email: friendEmail }).exec();
    if (!friendtoBe) {
      return res.status(400).json("User with the given email not found");
    }
    let { firstName, lastName } = friendtoBe;
    let friendFullName = `${firstName} ${lastName}`;
    if (loginEmail !== friendEmail) {
      let result = user.friends.find(
        (friend: friends) => friend.email === friendEmail
      );
      if (!result) {
        //check my friendlist
        try { await user.updateOne({ $push: { friends: { email: friendEmail, isFavourite: false, fullName: friendFullName, }, }, });
          await friendtoBe.updateOne({ $push: { friends: { email: loginEmail, isFavourite: false, fullName: userFullName, }, }, });
          return res.status(200).send({
            message: `You have successfully added to friend list the user with the email address`,
            newFriend: friendtoBe,
          });
         let friend: any = await FriendList.create({
            userId: _id,
            friend_id: friendtoBe._id,
            email: friendEmail,
            isFavorite: user.friends.some((unit: any) => {return (unit.isFavorite === true  && unit.email === friendEmail) }),
            firstName,
            lastName
          });
        } catch (err: any) {
...
export const addFavourite = async (Expressreq: Request, res: Response) => {
  ...
  try {
    ...
    if (loginEmail !== email) {
      let result = user.friends.find(
        (friend: friends) => friend.email === email
      ); //check my friendlist
      if (result) {
        if (result.isFavourite)
          ...
          return res
            .send({
              Status: "Success",
              Message: "User successfully added to your favorite list",
            })
            .status(200);
        }
      } else {
        ...
        return res
          .send({
            Status: "Fail",
            Message:
              "This user is not in your friends list, to make the user your favorite friend, please add the user to your friend list!",
          })
          .status(404);
      }
    } else {
...
export const getAllFavourites = async (Expressreq: Request, res: Response, next: NextFunction) => {
  ...
  try {
   ...
   const favoriteFriends: any = user.friends.filter((friend: friends) => friend.isFavourite != false).map((data: friends) => ({Name: data.fullName, email: data.email}))
   if (!favoriteFriends) return res.send({Message: "You don't have a list of favorite friend"})
    return res.send({Staus: 'Success', Favorite: favoriteFriends}).status(200)
  } catch (err: any) {
    res.status(500).json(err)
  }
}
